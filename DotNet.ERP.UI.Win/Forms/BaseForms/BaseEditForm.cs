...
using DotNet.ERP.UI.Win.UserControls.Grid;
...
namespace DotNet.ERP.UI.Win.Forms.BaseForms
{
    public partial class BaseEditForm : RibbonForm
    {
        ...
        private bool _kapanisIcinGeriAlindi;
        ...
        protected virtual void GeriAl(bool onaySor = true)
        {
            if (onaySor)
            {
        ...
        protected virtual DialogResult ZorunluKayitOnayi()
        {
            return Messages.ZorunluDevamKayitMesaji();
        }
        protected bool ZorunluKaydet()
        {
            GuncelNesneOlustur();
            ...
            {
                case IslemTuru.EntityInsert:
                    kaydedildi = EntityInsert();
                    break;

                case IslemTuru.EntityUpdate:
                    kaydedildi = EntityUpdate();
                    break;
            }
            ...
            // Insert sonrası OldEntity güncelle
            OldEntity = CurrentEntity;
            RefleshYapilacak = true;
            ButonEnabledDurumu();

            // INSERT’tan sonra update moduna geç
            BaseIslemTuru = IslemTuru.EntityUpdate;

            return true;
        }
        protected bool ZorunluKayitIleDevamEt()
        {
            if (BaseIslemTuru != IslemTuru.EntityInsert)
                return true;

            // Örnek: CariSube insert için kritik kontrol
            if (CurrentEntity is CariSube sube && sube.CariId == 0)
                return false;

            var sonuc = ZorunluKayitOnayi();

            if (sonuc == DialogResult.Yes)
                return ZorunluKaydet();

            return false;
        }
        protected virtual void GeriAlCore() 
        {         
            btnKaydet.Enabled = false;
            BaseIslemTuru = IslemTuru.EntityUpdate;
        }
