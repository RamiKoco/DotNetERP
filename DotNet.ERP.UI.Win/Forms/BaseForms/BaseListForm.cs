...
namespace DotNet.ERP.UI.Win.Forms.BaseForms
{
    public partial class BaseListForm : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        protected internal Functions.SelectRowFunctions RowSelect;
        ...
        {
            Functions.TablePrintingFunctions.Yazdir(Tablo, Tablo.ViewCaption, AnaForm.SubeAdi);
        ...
        private void Tablo_DoubleClick(object sender, EventArgs e)
        {
            ...
            var info = Tablo.CalcHitInfo(me.Location);

            // 🛑 Column header veya resize alanı
            if (info.HitTest == GridHitTest.Column ||
                info.HitTest == GridHitTest.ColumnEdge)
                return;

            // 🛑 Boş tabloya tıklama
            if (info.HitTest == GridHitTest.EmptyRow && Tablo.DataRowCount == 0)
                return;

            // 🛑 Data row değilse
            if (info.HitTest != GridHitTest.Row &&
                info.HitTest != GridHitTest.RowCell)
                return;
        ...
        //    var hitInfo = view.CalcHitInfo(me.Location);

        //    // 🛑 Band başlığı veya band resize alanı
        //    if (hitInfo.Band != null && !hitInfo.InRow)
        //        return;
        ...
        public static BaseListForm GetOpenForm(Type formType)
        {
